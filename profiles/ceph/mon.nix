...
in {
  ...
  systemd.services.ceph-mon-setup = {
    wantedBy = [ "multi-user.target" ];
    before = [ "ceph-mon-${nodeName}" ];
    path = with pkgs; [ systemd ceph gnugrep xfsprogs ];
    serviceConfig = {
      Type = "oneshot";
      ...
      ExecStart = pkgs.writeBashChecked "ceph-mon-setup.sh" ''
        ...
        if [ -s /var/lib/ceph/mgr/ceph-${nodeName}/keyring ]; then
          exit 0
        fi
        ...
        monmaptool \
          --clobber \
          --create \
          --add ${nodeName} ${config.cluster.instances.${nodeName}.privateIP} \
        ...
        mkdir -p /var/lib/ceph/mgr/ceph-${nodeName}/
        touch /var/lib/ceph/mon/ceph-${nodeName}/done
        ...
        > /var/lib/ceph/mgr/ceph-${nodeName}/keyring
        systemctl start ceph-mgr-${nodeName}
      '';
    };
  };
  ...
  secrets.generate.mon = ''
    export PATH="${
      lib.makeBinPath (with pkgs; [ coreutils sops toybox ceph jq ])
    }"
