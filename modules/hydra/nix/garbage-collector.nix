...
in {
  options.nix.gc = {
    ...
    absoluteTimedGB = lib.mkOption {
      ...
      default = 30;
      description =
        "The max absolute level to free to on the /nix/store mount for the timed GC";
    };
  };
  ...
  config.nix = lib.mkIf cfg.automatic {
    gc = {
      ...
      options = ''
        --max-freed "$((${
          toString cfg.absoluteTimedGB
        } * 1024**3 - 1024 * $(df -P -k /nix/store | tail -n 1 | ${pkgs.gawk}/bin/awk '{ print $4 }')))"'';
    };

    # This GC is run automatically by nix-build
    extraOptions = ''
      # Try to ensure between ${toString cfg.autoMinFreeGB}G and ${
        toString cfg.autoMaxFreedGB
      }G of free space by
